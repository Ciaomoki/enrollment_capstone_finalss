...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Bottom-nav navigation (explicit data-href set in HTML)
  const leftBtn = document.getElementById("navLeftBtn");
  const rightBtn = document.getElementById("navRightBtn");
  leftBtn?.addEventListener("click", (e) => {
    e.preventDefault();
    const href = leftBtn.getAttribute("data-href") || "studcheck.php";
    if (href) window.location.href = href;
  });
  rightBtn?.addEventListener("click", (e) => {
    e.preventDefault();
    const href = rightBtn.getAttribute("data-href") || "studenlist.php";
    if (href) window.location.href = href;
  });
  ...
  // ===== Steps Bar — snap to the ARROW ICON center; keeps in sync on X-scroll =====
  (function () {
  ...
  (function updateRowCount() {
    const countEl = document.getElementById("assessRowCount");
    const rows = document.querySelectorAll(".assessment-table tbody tr");
    if (countEl) countEl.textContent = `Subjects in assessment: ${rows.length}`;
  })();
});

// ================= Container drag-resize + X-scroll hint + Header Search (canonical) =================
(function initAssessmentResizeAndHint() {
  "use strict";
  if (window.__assessBound) return; // guard against duplicate binds
  window.__assessBound = true;

  document.addEventListener("DOMContentLoaded", function () {
    const card = document.querySelector(".assess-card");
    const handle = document.querySelector(".assess-resize-handle");
    const tableScroll = document.getElementById("assessTableScroll");
    const hint = document.getElementById("assessScrollHint");
    ...
    /* =========================================================
       Student Modules — Header Search (same behavior as Checklist)
       ========================================================= */
